                ...
                <div class="card-body">
                    ...
                    @if ($selectedRows)
                        <span class="mb-2 text-success">
                            ...
                            selected
                            <span class="text-dark font-weight-bold">{{ count($selectedRows) }}</span> {{ Str::plural('level', count($selectedRows)) }}
                            ...
                            <tbody>
                                @forelse ($levels as $level)
                                    <tr>
                                        <td scope="col">
                                            <div class="custom-control custom-checkbox small">
                                                <input type="checkbox" wire:model="selectedRows" value="{{ $level->id }}" class="custom-control-input" id="{{ $level->id }}">
                                                <label class="custom-control-label" for="{{ $level->id }}"></label>
                                            </div>
                                        </td>
                                        <td>{{ $loop->iteration }}</td>
                                        <td class="dtr-control sorting_1" tabindex="0">{{ $level->name }}</td>
                                        <td>
                                            <span  class="font-weight-bold badge text-white {{ $level->status == 1 ? 'bg-success' : 'bg-secondary' }}">
                                                {{ $level->status() }}
                                            </span>
                                        </td>
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <button wire:click.prevent="edit({{ $level }})" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></button>
                                                @if (auth()->user()->roles[0]->name == 'superadmin')
                                                    <button wire:click.prevent="confirmLevelRemoval({{ $level->id }})" class="btn btn-danger btn-sm"><i class="fa fa-trash bg-danger"></i></button>
                                                @else
                                                    <button class="btn btn-danger btn-sm" disabled><i class="fa fa-trash bg-danger"></i></button>
                                                @endif
                                            </div>
                                            {{-- <form action="" method="post" id="delete-level-{{ $level->id }}" class="d-none">
                                                @csrf
                                                @method('DELETE')
                                            </form> --}}
                                        </td>
    ...
    <div class="modal fade" id="confirmationModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer bg-light">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="mr-1 fa fa-times"></i> Cancel</button>
                    <button type="button" wire:click.prevent="deleteLevel" class="btn btn-danger"><i class="mr-1 fa fa-trash"></i>Delete Level</button>
                </div>
            </div>
        </div>
    </div>
